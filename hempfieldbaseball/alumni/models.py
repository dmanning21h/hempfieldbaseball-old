...
class AlumniClassManager(models.Manager):
    def get_queryset(self):
        return (
            ...
            .prefetch_related(
                Prefetch(
                    "players", AlumniPlayer.objects.select_related("info", "college")
                )
            )
        )
...
class AlumniPlayer(models.Model):
    ...
    alumni_class = models.ForeignKey(
        "AlumniClass",
    ...
    college_roster_photo = models.ImageField(
        upload_to="alumni-portraits", blank=True, null=True
    )
